...
resource "aws_instance" "thread" {
  ...
  user_data = base64encode(<<-EOF
    ...
    # Récupérer les IPs via AWS metadata et API
    ...
    docker run -d --name marinelangrez-thread \
      ...
      -e NUXT_PUBLIC_SENDER_URL=http://localhost:8080 \
      --restart unless-stopped \
...
resource "aws_instance" "sender" {
  ...
  user_data = base64encode(<<-EOF
    ...
    # Récupérer les IPs via AWS metadata
    SELF_IP=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)
    
    docker run -d --name marinelangrez-sender \
      -p 8080:3000 \
      -e NUXT_PUBLIC_API_URL=http://${aws_instance.api.public_ip}:3000 \
      -e NUXT_PUBLIC_THREAD_URL=http://localhost \
